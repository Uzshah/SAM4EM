...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        if is_second:
            ...
            self.norm2 = nn.LayerNorm(16384)
            self.mlp = MLPBlock(16384, 2048)
            self.med_sel = nn.Sequential(
    ...
    def forward(
        ...
        """
        masks, iou_pred, msk_feat, up_embedd = self.predict_masks(
        ...
        # Prepare output
        return masks, iou_pred, msk_feat, up_embedd
    ...
    def predict_masks(
        ...
        # if msk_feat is not None:
        #     print(msk_feat.shape)
        ...
        # Run the transformer
        hs, src = self.transformer(src, pos_src, tokens, mask_feat)
        iou_token_out = hs[:, s, :]
        mask_tokens_out = hs[:, s + 1 : (s + 1 + self.num_mask_tokens), :]
        # print(mask_tokens_out.shape)
        msk_tokens_out = hs[:, s + 0 : (s + 1 + self.num_mask_tokens), :]
        msk_feat = torch.matmul(msk_tokens_out, src.transpose(1, 2))
        ...
        if up_embedd is not None:
            upscaled_embedding = self.skip_connect(torch.cat((upscaled_embedding, up_embedd),dim=1))
            # print(torch.cat((upscaled_embedding, up_embedd),dim=1).shape, upscaled_embedding.shape)
